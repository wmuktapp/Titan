...
class AcquireProgram(object):
    def __init__(self):
        ...
        self._blob_service = blob.BlockBlobService(account_name=self._app.config["TITAN_AZURE_BLOB_ACCOUNT_NAME"],
        ...
        self.file_name_format = "{TITAN_DATA_SET_NAME}_{TITAN_LOAD_DATE}"
    ...
    def create_blob_from_stream(self, stream, count=None, **file_name_params):
        ...
        self._blob_service.create_blob_from_stream(self._app.config["TITAN_AZURE_BLOB_CONTAINER_NAME"], blob_name,
                                                   stream, count=count)
